...
<img src="../images/add_variable.png" alt="drawing" width="100%"/>
...
<p align="justify">
&ensp;&ensp;&ensp;&ensp;Let's image for some reason we had to change our staging area from Google Cloud Storage to AWS S3. Since we're using a variable, we don't have to change anything in the code, we'd just have to change the variable's value.
</p>
...
<br>
&ensp;&ensp;&ensp;&ensp;Just for you know, we if print the later variable, the bucket path, we'll something like this in the task log:
</p>

<img src="../images/variable_print.png" alt="drawing" width="100%"/>


<p align="justify">
&ensp;&ensp;&ensp;&ensp;So, let's create a new variable that contains 'password' in the key and let's see if it will be printed in the logs. Let's create the variable with the key 'user_password' and the value 123. Just by doing that, the UI automatically hides the value:
</p>

<img src="../images/variable_sensitive.png" alt="drawing" width="100%"/>

<p align="justify">
&ensp;&ensp;&ensp;&ensp;Lets then print that variable in a task and see what happens:
</p>

<img src="../images/sensitive_variable_print.png" alt="drawing" width="100%"/>

<p align="justify">
&ensp;&ensp;&ensp;&ensp;Woow! Pretty cool, isn't it? Well, I think those two example helped with better understanding of Airflow variables and senstive variables hidding using key words.
</p>

### Passing a json as the variable value
<p align="justify">
&ensp;&ensp;&ensp;&ensp;However, that's no the only cool thing we can do with variables. Let's image we have three variables related to something, let's say, taxi vehicle information: cab, vehicle age, and driver age. Would we have to create three different variable to store that information. Certainly not. Airflow allows us to pass a json as a varaible value:
</p>

<img src="../images/json_variable.png" alt="drawing" width="100%"/>
...
<p align="justify">
&ensp;&ensp;&ensp;&ensp;So, variables are such an amazing resource we can use, but they have some limitations we have to keep in mind. The first one is related to the parsing time of the scheduler. What happens is that when we fetch variable in a DAG we're creating a connection with the metadatabase. Then, if we fetch a DAG outside a task we'll end up with connections with the database being created every time the schedule parsers the DAG even if they're not being used. The snippet bellow better ilustrates this:
</p>
