...
<p align="justify">
&ensp;&ensp;&ensp;&ensp;There are three different ways we can interact with Apache Airflow: User Interface, Command Line Interface, and a Rest API. So, let's take a brief look at them.
</p>
...
<p align="justify">
&ensp;&ensp;&ensp;&ensp;Most of the times we'll use the UI for interaction. But there are some special cases when we need to use the CLI. According to <a href="https://airflow.apache.org/docs/apache-airflow/1.10.2/cli.html">the docs</a>, Airflow CLI allows many types of operations: starting processes; supporting developments; testing a DAG. Besides that, we also need the CLI when we need to initialize or to update Airflow.
</p>
...
<p align="justify">
&ensp;&ensp;&ensp;&ensp;Once we are logged in Airflow, the first view is the DAGs View. It lists all DAGs we have in our DAGs folder. Besides that, it brings a lot of information, such as the datetime of the last run, the schedule interval, allows us to trigger the DAG, and so on.
<br>

&ensp;&ensp;&ensp;&ensp;The image above shows us a DAGs View with an example DAG - etl_pipeline. There we have:
</p>

1. A toggle (to the left of the DAG's name): it defines whether or not the DAG is ready to be scheduled. It's important to keep it activated even if we only want to manually trigger the DAG.
2. The name of the DAG: the name that appears there is the name we setted when creating it.
...
9. Actions: allows us to manually trigger or to remove a DAG. The deletion does not delete the file that contains our DAG, it only deletes the metadata related to our DAG.
...
<p align="justify">
&ensp;&ensp;&ensp;&ensp;On the left, we have something similar to the Tree, but now, for each task, we also have its duration. On the right, the display will change dependening on our interaction: by default, it shows the DAG details, such as the total number of success and failures, first run, and etc.; if we click on some of that execution bars, it will display the execution details; and, if we click on a specific status box of a task instance, it will display the task details, such as the used Operator, last and next run.
</p>
...
<p align="justify">
&ensp;&ensp;&ensp;&ensp;In order to have a most comprehensive view of the tasks, Airflow has a graph that shows all dependencies between tasks and their current status for a specific run (usually, the latest).
</p>
...
<p align="justify">
&ensp;&ensp;&ensp;&ensp;When we click on some task instance, it will open a task interaction screen just like the image above that brings both task details and actions we can perform. Let's take a quickly look at some of those options:
</p>

1. Instance Details: brings some details about the task, such as the run duration, next run, metadata, and so on.
...
6. Tasks Actions:
    ...
    - Clear: it is important when we have a task failure. In this scenario, after fixing the error, we can just clear it in order to restart the task (retry).
